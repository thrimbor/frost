...
sub smp_init ()
	...
	if (floating_pointer = nullptr) then
		printk(LOG_INFO COLOR_GREEN "SMP: " COLOR_RESET !"floating pointer not found\n")
		return
	end if

	printk(LOG_INFO COLOR_GREEN "SMP: " COLOR_RESET !"floating pointer found: %X\n", cuint(floating_pointer))
	...
	if (floating_pointer->features(0) = 0) then
		...
		if (checksum(config_table, config_table->base_table_length) <> 0) then
			printk(LOG_INFO COLOR_GREEN "SMP: " COLOR_RESET !"checksum of the config table is wrong!\n")
			return
		end if
		...
		for entry_count as uinteger = 0 to config_table->entry_count-1 step 1
			select case *entry
				...
				case CT_ENTRY_TYPES.PROCESSOR:
					num_procs += 1
					printk(LOG_INFO COLOR_GREEN "SMP: " COLOR_RESET !"processor #%u found\n", num_procs)
				...
				case CT_ENTRY_TYPES.IO_APIC:
					...
					printk(LOG_INFO COLOR_GREEN "SMP: " COLOR_RESET !"I/O APIC found, ID: %X", cuint(ioapic->id))
				...
				case else:
					printk(LOG_INFO COLOR_GREEN "SMP: " COLOR_RESET !"config table entries corrupt!\n")
					return
			end select
		next
